...
useful_msg = {'/get_task': btn_classes.db_conn.db_get_task,
              ...
              '/notify_settings': btn_classes.notifies.notify_settings,
              '/not_notify': btn_classes.notifies.turn_off_notif,
              '/exist_notify': btn_classes.notifies.existing_notifies,
...
async def main_commands(message):
    ...
    greeting = (
        'Привет, тебя встречает команда reCREARTIVE \n\n'
        ...
        'Кстати, можешь накидать нам любимых стикеров, чтобы наше общение'
        ' было веселее!\n\nВыбирай!\n\n'
        ...
        ' - настроить уведомления (ты же не хочешь, чтобы мы тебя разбудили?'
        ' настрой часовой пояс и время отправки уведомлений) или узнать '
        'о уже выбранном времени\n/notify - включить уведомления \n'
        '/exist_notify - узнать о существующих уведомлениях (а вдруг'
        ' ты все уже настроил? )\n/not_notify - выключить уведомления'
        ' (после этого уведомления будут выключены)\n\n'
        '<strong>Стикеры</strong>\n'
        '/stickers - настроить любимые стикеры (данные стикеры будут приходить'
        ' вместе с остальными сообщениями)\n'
        '/add_stickers - добавить стикеры (после нажатия данной кнопки,'
        ' стикеры будут добавлены в вашу коллекцию)\n'
        '/del_stickers - удалить стикеры (когда они тебе надоели)\n'
        '/stop_stickers - остановить отправку стикеров (когда хочется'
        'поговорить серьезнее)\n'
...
@bot.callback_query_handler(
    func=lambda callback: callback.message.chat.type == 'private'
    )
...
@bot.callback_query_handler(
    func=lambda callback: callback.message.chat.type == 'supergroup'
    )
async def confirm_callback(callback):
    if callback.data == 'Подтвердить':
        ...
        user_name = (
            user_message[2].split(':')[-1][1:]
            if user_message[2].split(':')[-1][1:] != 'None'
            else None
            )
        user_surname = (
            user_message[3].split(':')[-1][1:]
            if user_message[3].split(':')[-1][1:] != 'None'
            else None
            )
        username = (
            user_message[4].split(':')[-1][1:]
            if user_message[4].split(':')[-1][1:] != 'None'
            else None
            )
        user_msg = (
            ''.join(user_message[5:-1])
            [''.join(user_message[5:-1])
             .index('Задание: ') + len('Задание: '):]
            )
        await btn_classes.db_conn.db_write_task(
            bot,
            callback.message,
            user_id,
            user_name,
...
async def reply_msg(reply):
    if (
        reply.reply_to_message is not None and
            u'\u274C' in reply.reply_to_message.text.split('\n')[-2]
    ):
        reason = (
        ...
        await bot.edit_message_text(
            ...
            message_id=reply.reply_to_message.id,
            text=reason,
            parse_mode='html'
            )
        markup = btn_classes.init_btns()
        ...
        text = (
            'Ваше сообщение не было одобрено,'
            f' модератор высказал причину:\n\n{reply.text}\n\n'
            )
        task = (
            '\n'.join(reply.reply_to_message.text.split('\n')[-3:-2])
            ['\n'.join(reply.reply_to_message.text.split('\n')[-3:-2])
...
@bot.message_handler(
    commands=[comm[1:] for comm in useful_msg], chat_types=['private']
    )
...
@bot.message_handler(
    content_types=['location', 'venue'], chat_types=['private']
    )
async def geo_location(message):
    btn_classes.stick.flag[message.from_user.id] = None
    if (message.location is not None and
            btn_classes.notifies.flag_location[message.chat.id]):
...
async def get_private_message(message):
    ...
    if btn_classes.stick.stick_sending.get(message.chat.id):
        ...
        await bot.send_message(
            message.chat.id,
            'Закончите отправку стикеров (кнопка "stop_stickers")',
            reply_markup=markup
            )
...
async def get_sticker_messages(sticker):
    ...
    if (btn_classes.stick.flag[sticker.from_user.id] is not None and
            btn_classes.stick.flag[sticker.from_user.id]):
        if data.get(str(sticker.from_user.id)):
            (data[str(sticker.from_user.id)]
             .update({
                sticker.sticker.file_unique_id: sticker.sticker.file_id
                }))
        else:
            data[str(sticker.from_user.id)] = ({sticker.sticker
                                               .file_unique_id: sticker
                                               .sticker.file_id}
                                               )
    elif (
        btn_classes.stick.flag[sticker.from_user.id] is not None
        and not btn_classes.stick.flag[sticker.from_user.id]
        and sticker.sticker.file_unique_id
        in data[str(sticker.from_user.id)]
    ):
